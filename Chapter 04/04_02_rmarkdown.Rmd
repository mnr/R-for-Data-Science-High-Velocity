...
Tests for conditions take time. It’s surprisingly complex to compare two variables, especially when R isn’t sure what is being compared or how to compare it.
R is forgiving about assigning values to variables. Unlike some other languages, it will let you change the type of the variable.

The cost of this flexibility is that every time R encounters a variable, it has to confirm what type it is and how to test it against another variable. 

This takes time. 

And that slows R down.
Let’s take a look at ways to fix the starting script so it doesn’t have to test each time it goes through the loop.
